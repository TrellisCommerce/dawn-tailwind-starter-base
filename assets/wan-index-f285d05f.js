...
function je() {
  ...
  if (a && t.loop) {
    ...
    if (t.slidesPerGroup > 1) {
      ...
      for (let b = 0; b < P; b += 1) g.push(g[g.length - 1] + T);
    }
...
function ut(i, e, t, s, n) {
  ...
  if (l.cssMode) {
    ...
    if (e === 0)
      ...
      if (!r.support.smoothScroll)
        return (
          we({ swiper: r, targetPosition: L, side: S ? 'left' : 'top' }), !0
        );
      m.scrollTo({ [S ? 'left' : 'top']: L, behavior: 'smooth' });
    }
    return !0;
  }
  return (
...
function yt(i) {
  ...
  typeof r > 'u'
    ? (r = a.getSlideIndex(
        u.filter((b) => b.classList.contains(m.slideActiveClass))[0],
      ))
    : (S = r);
  const L = s === 'next' || !s,
    A = s === 'prev' || !s;
  ...
  if (T < y) {
    z = Math.max(y - T, E);
    for (let b = 0; b < y - T; b += 1) {
      const x = b - Math.floor(b / C) * C;
      if (d) {
        const M = C - x - 1;
  ...
  if (
    ...
    m.slidesPerView === 'auto'
      ...
      : d &&
        ...
        a.slides.forEach((b, x) => {
          a.grid.updateSlide(x, b, a.slides);
        }),
    ...
    if (c.length > 0 && A) {
      if (typeof e > 'u') {
        const b = a.slidesGrid[S],
          M = a.slidesGrid[S + z] - b;
        l
          ? a.setTranslate(a.translate - M)
          : (a.slideTo(S + Math.ceil(z), 0, !1, !0),
            n &&
        ...
        const b = d ? c.length / m.grid.rows : c.length;
      ...
      if (typeof e > 'u') {
        ...
        const b = d ? w.length / m.grid.rows : w.length;
        a.slideTo(a.activeIndex - b, 0, !1, !0);
      }
  }
  if (
...
var xt = { loopCreate: bt, loopFix: yt, loopDestroy: Tt };
function Et(i) {
  const e = this;
  if (
        ...
        s.currentTranslate > t.minTranslate() &&
          ...
          n.resistance &&
            (s.currentTranslate =
              t.minTranslate() -
              1 +
              (-t.minTranslate() + s.startTranslate + h) ** S)))
      : h < 0 &&
        (y &&
          d &&
          !c &&
        ...
        s.currentTranslate < t.maxTranslate() &&
          ...
          n.resistance &&
            (s.currentTranslate =
              t.maxTranslate() +
              1 -
              (t.maxTranslate() - s.startTranslate - h) ** S))),
...
function zt(i) {
  ...
  for (
    ...
    const L = S < o.slidesPerGroupSkip - 1 ? 1 : o.slidesPerGroup;
    typeof u[S + L] < 'u'
      ? (h || (m >= u[S] && m < u[S + L])) && ((v = S), (E = u[S + L] - u[S]))
      : (h || m >= u[S]) && ((v = S), (E = u[u.length - 1] - u[u.length - 2]));
  }
  let y = null,
    d = null;
  o.rewind &&
  ...
  s('init', () => {
    ...
    s('click', (h, v) => {
      ...
      if (e.isElement && !c) {
        ...
        w && (c = w.find((S) => E.includes(S) || y.includes(S)));
      }
...
function Ce(i) {
  ...
  function p(d) {
    ...
    if (e.params.loop) {
      ...
      const S = f(e.realIndex, w, e.slides.length);
      S === 'next'
        ? e.slideNext()
        : S === 'previous'
        ? e.slidePrev()
  ...
  function g() {
    ...
    if (
      (e.params.loop
        ...
        ? ((S = e.snapIndex), (L = e.previousSnapIndex))
        : ((L = e.previousIndex || 0), (S = e.activeIndex || 0)),
      c.type === 'bullets' &&
      ...
      if (
        ...
        O.forEach((b) => {
          const x = Q(b);
          x === S
            ? b.classList.add(...c.bulletActiveClass.split(' '))
            : e.isElement && b.setAttribute('part', 'bullet'),
            c.dynamicBullets &&
              (x >= C &&
                x <= P &&
                b.classList.add(...`${c.bulletActiveClass}-main`.split(' ')),
              x === C && u(b, 'prev'),
              x === P && u(b, 'next'));
        });
      else {
        const b = O[S];
        if (
          (b && b.classList.add(...c.bulletActiveClass.split(' ')),
          e.isElement &&
            O.forEach((x, M) => {
              x.setAttribute('part', M === S ? 'bullet-active' : 'bullet');
            }),
          c.dynamicBullets)
        ) {
          const x = O[C],
      ...
      if (c.dynamicBullets) {
        const b = Math.min(O.length, c.dynamicMainBullets + 4),
          x = (o * b - o) / 2 - T * o,
          M = d ? 'right' : 'left';
        O.forEach((I) => {
      ...
      if (
        (c.type === 'fraction' &&
          (O.querySelectorAll(W(c.currentClass)).forEach((P) => {
            P.textContent = c.formatFractionCurrent(S + 1);
          }),
  ...
  function m() {
    ...
    if (d.type === 'bullets') {
      ...
      for (let A = 0; A < L; A += 1)
        d.renderBullet
          ? (S += d.renderBullet.call(e, A, d.bulletClass))
          : (S += `<${d.bulletElement} ${
    ...
    typeof d.el == 'string' && e.isElement && (c = e.el.querySelector(d.el)),
      ...
      !(!c || c.length === 0) &&
        (e.params.uniqueNavElements &&
          ...
          c.length > 1 &&
            (c = c.filter((w) => Se(w, '.swiper')[0] === e.el)[0])),
    ...
    s('click', (d, c) => {
      ...
      if (
        ...
        if (
          ...
          return;
        const L = S[0].classList.contains(e.params.pagination.hiddenClass);
...
function Me(i) {
  ...
  function m() {
    ...
    const { scrollbar: T, rtlTranslate: b } = e,
      { dragEl: x, el: M } = T,
  ...
  function y(T) {
    const { scrollbar: b, rtlTranslate: x } = e,
      { el: M } = b;
    let I;
    (I =
      (E(T) -
  ...
  function d(T) {
    ...
    (o = !0),
      ...
      b.hide && (I.style.opacity = 1),
      e.params.cssMode && (e.wrapperEl.style['scroll-snap-type'] = 'none'),
      n('scrollbarDragStart', T);
  }
  function c(T) {
    const { scrollbar: b, wrapperEl: x } = e,
      { el: M, dragEl: I } = b;
    o &&
  ...
  function z() {
    ...
    e.params.uniqueNavElements &&
      typeof x.el == 'string' &&
      M.length > 1 &&
      b.querySelectorAll(x.el).length === 1 &&
      (M = b.querySelector(x.el)),
      M.length > 0 && (M = M[0]),

...
function _(s, e) {
  return G().getComputedStyle(s, null).getPropertyValue(e);
}
function Q(s) {
  let e = s,
    t;
        ...
        ((r = new ResizeObserver((p) => {
          o = n.requestAnimationFrame(() => {
            ...
            let h = g,
              v = m;
            p.forEach((E) => {
              let { contentBoxSize: T, contentRect: d, target: c } = E;
              (c && c !== e.el) ||
                ((h = d ? d.width : (T[0] || T).inlineSize),
                (v = d ? d.height : (T[0] || T).blockSize));
            }),
              (h !== g || v !== m) && l();
          });
        })),
...
function je() {
      ...
      if (t.slidesPerView === 'auto') {
        ...
        if (
          ...
          const M = e(y, 'width'),
            ...
            F = e(y, 'margin-left'),
            D = e(y, 'margin-right'),
            ce = y.getPropertyValue('box-sizing');
  ...
  if (a && t.loop) {
    ...
    if (t.slidesPerGroup > 1) {
      ...
      for (let S = 0; S < P; S += 1) g.push(g[g.length - 1] + y);
    }
  ...
  if ((g.length === 0 && (g = [0]), c !== 0)) {
    ...
    f.filter((P, y) =>
      !t.cssMode || t.loop ? !0 : y !== f.length - 1,
    ).forEach((P) => {
      P.style[C] = `${c}px`;
    });
  }
  ...
  if (
    ...
    const C = -s.snapGrid[0],
      P = -s.slidesGrid[0];
    (s.snapGrid = s.snapGrid.map((y) => y + C)),
      (s.slidesGrid = s.slidesGrid.map((y) => y + P));
  }
  if (
...
function ut(s, e, t, i, n) {
  ...
  if (
    ...
    ((!r.allowSlideNext &&
      (g
        ? T > r.translate && T > r.minTranslate()
        : T < r.translate && T < r.minTranslate())) ||
      (!r.allowSlidePrev &&
        T > r.translate &&
        T > r.maxTranslate() &&
    ...
    return !1;
  o !== (f || 0) && t && r.emit('beforeSlideChangeStart'), r.updateProgress(T);
  let d;
  o > p ? (d = 'next') : o < p ? (d = 'prev') : (d = 'reset');
  const c = r.virtual && r.params.virtual.enabled;
  if (!(c && n) && ((g && -T === r.translate) || (!g && T === r.translate)))
    return (
      r.updateActiveIndex(o),
      l.autoHeight && r.updateAutoHeight(),
      r.updateSlidesClasses(),
      l.effect !== 'slide' && r.setTranslate(T),
...
function Tt(s) {
  ...
  const C = d ? Math.ceil(u.length / m.grid.rows) : u.length,
    y = (d ? u[r].column : r) + (h && typeof n > 'u' ? -v / 2 + 0.5 : 0);
  if (y < T) {
    z = Math.max(T - y, E);
    for (let S = 0; S < T - y; S += 1) {
...
var xt = { loopCreate: St, loopFix: Tt, loopDestroy: yt };
function Et(s) {
  const e = this;
  if (
...
function It(s) {
  ...
  if (!i.isMoved) {
    if (
      (T && d && t.loopFix({ direction: t.swipeDirection }),
...
function zt(s) {
  ...
  if (g > o.longSwipesMs) {
    ...
    e.swipeDirection === 'next' &&
      (c >= o.longSwipesRatio
        ? e.slideTo(o.rewind && e.isEnd ? T : v + w)
        : e.slideTo(v)),
    ...
    (i.target === e.navigation.nextEl || i.target === e.navigation.prevEl)
      ? i.target === e.navigation.nextEl
        ? e.slideTo(v + w)
        : e.slideTo(v)
      : (e.swipeDirection === 'next' && e.slideTo(T !== null ? T : v + w),
...
const he = (s, e) => s.grid && e.grid && e.grid.rows > 1;
function Vt() {
  const s = this,
...
class B {
  constructor() {
    ...
    (l.__swiper__ = !0),
      (l.support = Se()),
      (l.device = Te({ userAgent: t.userAgent })),
      (l.browser = Ne()),
      (l.eventsListeners = {}),
...
function Ee(s) {
  ...
  function o(h, v) {
    ...
    (h = k(h)),
      h.forEach((T) => {
        T &&
          (T.classList[v ? 'add' : 'remove'](...E.disabledClass.split(' ')),
          T.tagName === 'BUTTON' && (T.disabled = v),
          e.params.watchOverflow &&
            e.enabled &&
            T.classList[e.isLocked ? 'add' : 'remove'](E.lockClass));
      });
  }
  function l() {
  ...
  function f() {
    ...
    v.forEach((d) => T(d, 'next')), E.forEach((d) => T(d, 'prev'));
  }
  function p() {
    let { nextEl: h, prevEl: v } = e.navigation;
    (h = k(h)), (v = k(v));
    const E = (T, d) => {
      T.removeEventListener('click', d === 'next' ? u : a),
        T.classList.remove(...e.params.navigation.disabledClass.split(' '));
    };
    h.forEach((T) => E(T, 'next')), v.forEach((T) => E(T, 'prev'));
  }
  i('init', () => {
    ...
    i('click', (h, v) => {
      let { nextEl: E, prevEl: T } = e.navigation;
      (E = k(E)), (T = k(T));
      const d = v.target;
      let c = T.includes(d) || E.includes(d);
      if (e.isElement && !c) {
        const w = v.path || (v.composedPath && v.composedPath());
        w && (c = w.find((b) => E.includes(b) || T.includes(b)));
      }
      if (e.params.navigation.hideOnClick && !c) {
        ...
        E.length
          ...
          : T.length &&
            (w = T[0].classList.contains(e.params.navigation.hiddenClass)),
...
function Ce(s) {
  ...
  i('changeDirection', () => {
    ...
    i('init', () => {
      e.params.pagination.enabled === !1 ? T() : (h(), m(), g());
    }),
    i('activeIndexChange', () => {
...
function Me(s) {
  ...
  function m() {
    ...
    const { scrollbar: y, rtlTranslate: S } = e,
      { dragEl: x, el: M } = y,
  ...
  function d(y) {
    ...
    (o = !0),
      (u =
        y.target === V
          ? E(y) -
            y.target.getBoundingClientRect()[e.isHorizontal() ? 'left' : 'top']
          : null),
      y.preventDefault(),
      y.stopPropagation(),
  ...
  function b(y) {
    ...
    if (!I) return;
    const D = y === 'on' ? 'addEventListener' : 'removeEventListener';
    I[D]('pointerdown', d, V),
      r[D]('pointermove', c, V),
      r[D]('pointerup', w, F);
  }
  ...
  function O() {
    ...
    S &&
      S.classList.remove(
        ...N(e.isHorizontal() ? y.horizontalClass : y.verticalClass),
      ),
      A();
  }
  i('changeDirection', () => {
